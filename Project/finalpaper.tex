...
The primary issue with writing constraints of this form is this is that they are possibly quadratic in size, which makes it unlikely for them to yield useful approximation techniques. It turns out that we can simplify the constraints to be linear in size, as we show in Section 4, and this seems more promising since the dynamic optimality problem is focused on constant factor approximations. 
